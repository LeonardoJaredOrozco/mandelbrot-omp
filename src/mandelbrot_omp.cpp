/*
 * mandelbrot_omp.cpp
 * Línea Base Paralela — generada con asistencia de IA
 * Paralelización con OpenMP de Tarea A y Tarea B
 */
...
// ---------------------------------------------------------------------------
// TAREA A — paralela con OpenMP (schedule static por defecto)
// ---------------------------------------------------------------------------
void generate_mandelbrot(std::vector<Pixel>& img) {
    ...
    std::cout << "[Tarea A] Tiempo paralelo: " << elapsed << " s\n";
}
...
void apply_gaussian_blur(const std::vector<Pixel>& src,
    ...
    auto t1 = std::chrono::high_resolution_clock::now();
    double elapsed = std::chrono::duration<double>(t1 - t0).count();
    std::cout << "[Tarea B] Tiempo paralelo: " << elapsed << " s\n";
}

// ---------------------------------------------------------------------------
// HISTOGRAMA — race condition
// ---------------------------------------------------------------------------
void compute_histogram(const std::vector<Pixel>& img,
...
int main() {
    ...
    std::cout << "Hilos disponibles: " << nthreads << "\n";
    std::cout << "Resolución: " << WIDTH << "×" << HEIGHT << "\n\n";

    std::vector<Pixel> img(WIDTH * HEIGHT);
    std::vector<Pixel> blurred(WIDTH * HEIGHT);
    std::vector<double> kernel;
    std::vector<int> hist_r, hist_g, hist_b;

    build_gaussian_kernel(kernel);
    generate_mandelbrot(img);
    apply_gaussian_blur(img, blurred, kernel);
    compute_histogram(blurred, hist_r, hist_g, hist_b);
    save_ppm("mandelbrot_omp.ppm", blurred);

    // Muestra los primeros 5 valores del histograma
    std::cout << "\nHistograma R (primeros 5 valores):\n";
    for (int i = 0; i < 5; ++i)
        std::cout << "  R[" << i << "] = " << hist_r[i] << "\n";
